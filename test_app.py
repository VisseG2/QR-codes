...
def test_user_sync_with_dates(client):
    save_devices({'d1': {'registered': True, 'last_seen': 'now'}})
    resp = client.post('/users/create', data={
        'uid': '2',
        'card': '2222',
        'start': '2025-01-02 12:34',
        'end': '2025-12-31 23:59'
    })
    assert resp.status_code == 302
    resp = client.get('/iclock/getrequest?SN=d1')
    body = resp.get_data(as_text=True)
    assert 'StartTime=20250102' in body
    assert 'EndTime=20251231' in body
